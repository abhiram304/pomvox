...
/// The status item (M7a): Murmur's resident face once the Hub window closes.
...
struct MenuBarIcon: View {
    ...
    var body: some View {
        Image(systemName: symbol)
            .accessibilityLabel("Murmur — \(shortStatus)")
    }
...
struct MenuBarContent: View {
    ...
    var body: some View {
        ...
        if needsAttention {
            Button("Open Setup…") {
                openHub()
                NotificationCenter.default.post(name: .murmurShowSetup, object: nil)
            }
        }
        Divider()
...
extension Notification.Name {
    ...
    static let murmurShowSetup = Notification.Name("app.murmur.showSetup")
}
