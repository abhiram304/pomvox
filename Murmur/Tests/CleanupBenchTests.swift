...
/// The M0 `murmur-bench-llm` benchmark, relocated inside the Xcode app target —
...
///
/// Skipped unless MURMUR_LLM_BENCH=1 — it loads the real ~2.3 GB model:
///   TEST_RUNNER_MURMUR_LLM_BENCH=1 DEVELOPER_DIR=... xcodebuild test ... \
///     -only-testing:MurmurTests/CleanupBenchTests
final class CleanupBenchTests: XCTestCase {
    ...
    static let fixtures: [(name: String, text: String)] = [
        ...
        (
            "long_15s",
            "okay so here's the plan for the murmur project. first we benchmark "
    ...
    func testBenchAgainstPythonGate() async throws {
        try XCTSkipUnless(
            ...
            "set TEST_RUNNER_MURMUR_LLM_BENCH=1 to run the cleanup LLM bench")
