...
final class MurmurDictionaryTests: XCTestCase {
    ...
    // MARK: - MurmurDictionary

    func testDictionaryAppliesReplacementsAndExposesHint() {
        let d = MurmurDictionary(words: ["Murmur"], replacements: [("mur mur", "Murmur")])
        XCTAssertTrue(d.hint.contains("Murmur"))
        XCTAssertEqual(d.apply("open mur mur now"), "open Murmur now")
    }
    ...
    func testDictionaryEmptyTextPassthrough() {
        let d = MurmurDictionary(words: [], replacements: [("a", "b")])
        XCTAssertEqual(d.apply(""), "")
    }

    func testDictionaryNoReplacementsReturnsTextUnchanged() {
        let d = MurmurDictionary(words: ["Murmur"], replacements: [])
