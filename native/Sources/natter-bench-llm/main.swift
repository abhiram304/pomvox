// natter-bench-llm — M0 spike, cleanup-LLM half. Times Qwen3-4B-4bit via
...
//
//   swift run -c release natter-bench-llm [--out path.json]
//
// Needs the Metal toolchain (full Xcode). Run alone for clean numbers, or
// concurrently with natter-bench to demonstrate ANE/GPU isolation — the
...
let UTTERANCES = [
    ...
    "long_15s":
        "Okay so here's the plan for the natter project. First we benchmark the new speech model on the neural engine and compare it against the current pipeline. Then if the numbers hold up we port the hotkey state machine and the endpoint detector, keeping the python tests as the specification. Finally we wire up the cleanup model and measure the end to end latency against the budget in the spec document.",
]

var outPath = "/tmp/natter-native-swift-llm.json"
