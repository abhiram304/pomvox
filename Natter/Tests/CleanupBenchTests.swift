...
/// The M0 `natter-bench-llm` benchmark, relocated inside the Xcode app target —
...
///
/// Skipped unless NATTER_LLM_BENCH=1 — it loads the real ~2.3 GB model:
///   TEST_RUNNER_NATTER_LLM_BENCH=1 DEVELOPER_DIR=... xcodebuild test ... \
///     -only-testing:NatterTests/CleanupBenchTests
final class CleanupBenchTests: XCTestCase {
    ...
    static let fixtures: [(name: String, text: String)] = [
        ...
        (
            "long_15s",
            "okay so here's the plan for the natter project. first we benchmark "
    ...
    func testBenchAgainstPythonGate() async throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["NATTER_LLM_BENCH"] == "1",
            "set TEST_RUNNER_NATTER_LLM_BENCH=1 to run the cleanup LLM bench")
    ...
    func testPhase3AcceptanceParityAndTimeoutFallback() async throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["NATTER_LLM_BENCH"] == "1",
            "set TEST_RUNNER_NATTER_LLM_BENCH=1 to run the cleanup LLM acceptance")
