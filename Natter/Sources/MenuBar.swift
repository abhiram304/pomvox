...
/// The status item (M7a): Natter's resident face once the Hub window closes.
...
struct MenuBarIcon: View {
    ...
    var body: some View {
        Image(systemName: symbol)
            .accessibilityLabel("Natter — \(shortStatus)")
    }
...
struct MenuBarContent: View {
    ...
    var body: some View {
        ...
        if needsAttention {
            Button("Open Setup…") {
                openHub()
                NotificationCenter.default.post(name: .natterShowSetup, object: nil)
            }
        }
        Divider()
        Toggle("Use the native engine", isOn: engineBinding)
        Divider()
        Button("Quit Natter") { NSApp.terminate(nil) }
            .keyboardShortcut("q")
    }
...
extension Notification.Name {
    ...
    static let natterShowSetup = Notification.Name("app.natter.showSetup")
}
