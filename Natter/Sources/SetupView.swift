...
/// for Natter.app's own TCC grants. Three permission rows with a plain-language
...
struct SetupView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(complete ? "You're set — try the self-test" : "Natter setup")
    ...
    private var selfTestCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("Click the button, then watch Natter type into this box — the same ⌘V path a dictation uses.")
