...
/// Read-only view over the Python app's `~/.natter/history.db`. A separate
...
///
/// Pure of SwiftUI; unit-tested against fixture databases (point `NATTER_DB_PATH`
/// at one). Stat definitions are the spec the Python History window must agree
/// with — see NatterTests.
struct HistoryReader {
    let path: String

    /// Default location, overridable for tests via `NATTER_DB_PATH`.
    static func defaultPath() -> String {
        if let override = ProcessInfo.processInfo.environment["NATTER_DB_PATH"], !override.isEmpty {
            return override
        }
        return NSString(string: "~/.natter/history.db").expandingTildeInPath
    }
