...
The history UI now lives in the native Hub (Pomvox.app, M3), which reads this
...
DEFAULT_PATH = Path.home() / ".pomvox" / "history.db"
...
class HistoryStore:
    ...
    def __init__(self, path: Path | str = DEFAULT_PATH, retention_days: int = 7):
        ...
        # separate process (Pomvox.app, M1). Bump only with a coordinated change
