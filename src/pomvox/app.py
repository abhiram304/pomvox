...
def preview(text: str, limit: int = 60) -> str:
    ...
    ~/.pomvox/pomvox.log (unbounded, on by default); DEBUG carries them."""
    if len(text) <= limit:
        return text
    return text[: limit - 1] + "…"
...
class Controller:
        ...
        if not CONFIG_PATH.exists():
            ...
            if example.exists():
                ...
                CONFIG_PATH.write_text(
                    "# Pomvox configuration — every key is optional.\n"
                    "# Reference: config.example.toml in the Pomvox repo.\n"
                )
    ...
    def _open_hub(self) -> None:
        # Launch the native Hub (Pomvox.app) — a separate process that reads
        # ~/.pomvox/history.db read-only, so it never touches the dictation
        # path. Looks for an installed app first, then a local dev build.
        import subprocess

        candidates = [
            "/Applications/Pomvox.app",
            str(Path.home() / "Applications" / "Pomvox.app"),
            "/tmp/pomvox-hub-dd/Build/Products/Debug/Pomvox.app",
        ]
        for app_path in candidates:
            if Path(app_path).exists():
                subprocess.run(["open", app_path], check=False, timeout=5)
                return
        log.warning("hub: Pomvox.app not found (build it from Pomvox/ — see README)")
    ...
    def _try_install_tap(self) -> bool:
        ...
        if blocker is not None:
            log.error(
                "hotkey: another Pomvox engine holds the tap (%s, pid %d) — "
                "quit it before starting the Python engine",
                blocker.name, blocker.pid,
            )
            self._post_state("setup", "another Pomvox engine is running")
            return False
        try:
            self.tap.start()
