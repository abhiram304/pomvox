...
The native Swift engine (Natter.app) and the Python engine must never both hold
a CGEventTap or the microphone. A single file ``~/.natter/engine.pid`` records
...
``Natter/Sources/Engine/Pidfile.swift``): line 1 is the pid, line 2 the owner
...
PIDFILE = Path.home() / ".natter" / "engine.pid"
