...
MARKER_PATH = Path.home() / ".natter" / "onboarded"

PERMISSIONS = (
    ("microphone", "Microphone", "so Natter can hear you"),
    ("input_monitoring", "Input Monitoring", "so the hotkey works in every app"),
    ("accessibility", "Accessibility", "so Natter can type your words for you (⌘V)"),
)

RELAUNCH_NOTE = "granted — relaunch Natter to pick it up"
STALE_TCC_HINT = (
...
class OnboardingWindow:
    ...
    def refresh(self, rows: list[Row], complete: bool) -> None:
        ...
        title = "You're set — try the self-test" if complete else "Natter setup"
    ...
    def _build(self) -> None:
        ...
        label("Welcome to Natter", 24, h - 52, w - 48, size=20.0, bold=True)
        label(
        ...
        y -= 46
        label("Self-test — click, then watch Natter type into this box:",
