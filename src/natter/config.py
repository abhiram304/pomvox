"""Configuration loading for Natter.

Reads ``~/.natter/config.toml`` with stdlib :mod:`tomllib`. Every key has a
...
@dataclass(frozen=True)
class EngineConfig:
    # The native Swift engine (Natter.app) is off by default and owned by the
...
def load(path: Path | None = None) -> Config:
    """Load config from *path* (default ``~/.natter/config.toml``)."""
