...
def setup_logging(to_file: bool) -> None:
    ...
    if to_file:
        try:
            ...
            handlers.append(logging.FileHandler(CONFIG_DIR / "murmur.log"))
        except OSError as exc:
            print(f"murmur: cannot open log file ({exc})", file=sys.stderr)
    logging.basicConfig(
        level=logging.INFO,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="murmur", description="Fully local voice dictation for macOS."
    )
    parser.add_argument(
        "--version", action="version", version=f"murmur {__version__}"
    )
    parser.add_argument(
    ...
    if sys.platform != "darwin":
        print("murmur runs on macOS (Apple Silicon) only.", file=sys.stderr)
        return 1
