...
The endgame is a native Swift `Pomvox.app`: SwiftUI menu bar + Hub window,
...
gathered with `native/` (the `pomvox-bench` harness) and
...
pomvox-bench`. Python harness: `uv run python scripts/native_baseline.py`,
...
planned, the benchmark moved inside `Pomvox.app` (`CleanupBenchTests`, gated
on `POMVOX_LLM_BENCH=1`), where mlx-swift's metallib is a standard build
...
`Pomvox.app`) is an **Xcode app target**, where Metal-shader compilation and
...
cd native
swift run -c release pomvox-bench fixtures --out /tmp/swift-stt.json   # STT — runs from CLI
```

The STT harness (`pomvox-bench`, FluidAudio/CoreML) runs from a plain SwiftPM
build. The cleanup harness (`pomvox-bench-llm`, mlx-swift) **compiles** the
